...
def check_blinka_python_version():
    ...
    if int(current_major) >= int(required_major) and int(current_minor) >= int(
        required_minor
    ):
        return

    shell.bail(
        "Blinka requires a minimum of Python version {} to install, current one is {}. Please update your OS!".format(
            blinka_minimum_python_version, current
        )
    )
...
def set_raspiconfig():
    ...
    if not shell.run_command(
        "sudo raspi-config nonint do_serial_hw 0", suppress_message=True
    ):
